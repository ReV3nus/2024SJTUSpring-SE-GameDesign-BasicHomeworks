...
class ExplicitMidpointPendulum : MonoBehaviour
{
    ...
    void UpdatePosition()
    {
		...
		float half_omega = ome - (g / length) * Mathf.Sin(theta) * totalTime / 2;
		float half_theta = the + totalTime / 2 * ome;
        
        // 3. update theta
		theta = the + totalTime * half_omega;
        // 4. update omega
		omega = ome - (g / length) * Mathf.Sin(half_theta) * totalTime;
